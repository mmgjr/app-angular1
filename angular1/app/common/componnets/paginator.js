    ...
    angular.module('primeiraApp').component('paginator',{
        ...
        controller:[
            ...
            function($location){
                this.$onInit = function(){
                    ...
                    this.pagesArray = Array(pages).fill(0).map((element,index) => index + 1)
                }
        ...
        template:`
            <ul ng-if="$ctrl.needPagination" class="pagination pagination-sm no-margin pull-right">
                ...
                <li ng-class="{active: $ctrl.isCurrent(index)}" ng-repeat="index in $ctrl.pageArray">
                    <a href="{{ $ctrl.url }}?page={{ index }}">
                        {{index}}
                    </a>    
                </li>    
